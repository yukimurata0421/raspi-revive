...
| `TELEMETRY_PIPELINE_FAILURE` | (host heartbeat stale + sentinel stale + gpio fresh + ssh ok) OR (host heartbeat fresh-but-not-progressing + sentinel stale + gpio fresh + ssh ok). Internal reason code is split into `TELEMETRY_SOURCE_FAILURE` / `TELEMETRY_EXPORT_FAILURE` / `TELEMETRY_PULL_FAILURE`. | `NO_ACTION` | always allowed |
...
- `intervention-evidence/intervention_evidence_*.json` (evidence bundle before intervention)
- `incident-summary.json` (latest incident/decision snapshot)
- `controller-stats.json` (controller runtime counters and state/action summary)
...
## Deployment Preflight (Controller)

Before restarting `raspi-revive-controller.service`, run the import-consistency preflight:
...
The bundled unit template also runs this as `ExecStartPre` and uses `RestartPreventExitStatus=75`.
When preflight fails, restart is blocked until source consistency is restored.

Use atomic release deployment rather than direct in-place copy:
...
- Use `RASPI_REVIVE_REMOTE_REBOOT_WEBHOOK_URL` via `notify.remote_reboot_discord_webhook_url_env` for REMOTE_REBOOT execution notifications.
